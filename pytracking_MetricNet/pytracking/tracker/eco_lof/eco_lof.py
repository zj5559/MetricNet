...
from pytracking.metricnet.judge_metric_v2 import *
...
class ECO(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        with torch.no_grad():
            ...
            # warmup end
            self.target_metric_feature = get_target_feature(self.metric_model, np.array(state), np.array(image))
        pos_generator = SampleGenerator('gaussian', np.array([image.shape[1], image.shape[0]]), 0.1, 1.3)
        gt_pos_examples = pos_generator(np.array(state).astype(np.int), 20, [0.7, 1])
        gt_iou = 0.7
        while gt_pos_examples.shape[0] == 0:
            gt_iou = gt_iou - 0.1
            gt_pos_examples = pos_generator(np.array(state).astype(np.int), 20, [gt_iou, 1])
        # print('gt-iou：', gt_iou)
        # self.gt_pos_features = get_anchor_feature(self.metric_model, np.array(image), gt_pos_examples).cpu().detach().numpy()
        with torch.no_grad():
            gt_pos_features0 = get_anchor_feature(self.metric_model, np.array(image), gt_pos_examples)
    ...
    def track(self, image) -> dict:
        ...
        # metric
        state_tmp = torch.cat((self.pos[[1, 0]] - (self.target_sz[[1, 0]] - 1) / 2, self.target_sz[[1, 0]]))
        state_tmp = state_tmp.numpy()
        with torch.no_grad():
            self.current_target_metric_feature.append(get_target_feature(self.metric_model, state_tmp, np.array(image)).cpu().detach().numpy())
        ...
        if self.frame_num == 1:
            ...
            current_target_metric_feature = np.array(self.current_target_metric_feature).squeeze()
            current_target_metric_feature0 = torch.from_numpy(current_target_metric_feature).cuda()
            ...
            lof_predict, success = lof(current_target_metric_feature, self.clf, k=5, thresh=self.lof_thresh)
            last_id = -1
            ...
            for ii in range(len(self.train_xf)):
                ...
                if self.frame_num > self.params.train_skipping + 1 and success[ii]:
                    for kk in range(len(self.target_features_all) - 1, -1, -1):
                        dist = torch.norm(self.target_features_all[kk] - current_target_metric_feature0[ii].reshape([1,1024]), 2, dim=1).view(-1)
                        if dist < self.similar:
                            success[ii] = 0
                            continue
                if self.frame_num <= self.params.train_skipping + 1 or success[ii]:
                    self.target_features_all.append(current_target_metric_feature0[ii].reshape([1,1024]))
                    last_id = ii
                    self.update_memory(self.train_xf[ii])
            if last_id > -1:
                self.filter_optimizer.run(self.params.CG_iter, self.train_xf[last_id])
                self.symmetrize_filter()
            self.current_target_metric_feature = []
            self.train_xf = []
            # self.iou=[]
