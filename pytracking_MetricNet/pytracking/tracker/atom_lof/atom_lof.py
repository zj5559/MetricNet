...
class ATOM(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        with torch.no_grad():
            ...
            # self.similar =self.params.sim_rate
    ...
    def track(self, image) -> dict:
        ...
        if self.use_iou_net and flag != 'not_found':
            ...
            pos_tmp = self.pos
        state_tmp = torch.cat((pos_tmp[[1, 0]] - (self.target_sz[[1, 0]] - 1) / 2, self.target_sz[[1, 0]]))
        state_tmp = state_tmp.numpy()
        with torch.no_grad():
            current_target_metric_feature0 = get_target_feature(self.metric_model, state_tmp, np.array(image))
            current_target_metric_feature=current_target_metric_feature0.cpu().detach().numpy()
        # success, target_dist = judge_success(self.metric_model, current_target_metric_feature,
        #                                      self.target_metric_feature, self.params)
        lof_score, success = lof(current_target_metric_feature, self.clf, k=5, thresh=self.lof_thresh)

        if self.frame_num <= self.params.train_skipping:
            self.lof_thresh = (self.lof_thresh * (self.frame_num - 2) + lof_score * self.params.lof_rate) / (
                        self.frame_num - 1)
            if self.frame_num == self.params.train_skipping:
                print('lof-thresh', self.lof_thresh)
        ...
        out = {'target_bbox': new_state.tolist(),'max_score': max_score}
        return out
