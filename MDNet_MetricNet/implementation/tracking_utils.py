...
def compute_overall_results(dataset, result_path, save_path, eval_mode=None):
    ...
    if dataset == 'otb':
        ...
        base_dir = '/media/zj/4T/Dataset/LaSOT/dataset/images/'
        file = open('/media/zj/4T/Dataset/LaSOT/dataset/images/test2.txt', 'r')
        videos = file.readlines()
        # videos = os.listdir(result_path)
        # videos.sort()
        results = np.zeros((len(videos), 4))
        for id, video in enumerate(videos):
            video = video.strip('\n')  # airplane-1
            # video=video.split('.')
            # video=video[0]
            ...
            tmp = video.split('-')
            video_dir = tmp[0]
            seq_path = os.path.join(base_dir, video_dir, video)
            gt_path = os.path.join(seq_path, 'groundtruth.txt')
            bbox_path = os.path.join(result_path, video + '.txt')
            with open(gt_path) as f:
                gt = np.loadtxt((x.replace('\t', ',') for x in f), delimiter=',')
            with open(bbox_path) as f:
                # bbox = np.loadtxt((x.replace('\t', ' ') for x in f), delimiter=' ')
                bbox = np.loadtxt((x.replace('\t', ',') for x in f), delimiter=',')
            results[id] = _compile_results(gt, bbox, 20)
            g.writelines([str(id) + ' -- ' + video +
                          ' -- Precision: ' + "%.2f" % results[id, 1] +
    ...
    g.writelines(['-- Overall stats (averaged per frame) on ' + str(
        len(videos)) + ' -- Precision ' + "(20 px)" + ': ' + "%.2f" % overall_pre +
                  ...
                  ' -- IOU: ' + "%.2f" % overall_iou + ' --\n'])
    g.close()
    return overall_pre,overall_auc,overall_iou


def _init_video(video):
    root_dataset = '/home/daikenan/dataset/VOT18_long/'
    video += '/'
...
def show_boxes(im, boxes, step=1, gt=False):
    import matplotlib.patches as patches
    if gt:
        boxes = np.array([boxes[:, 1], boxes[:, 0], boxes[:, 3] + boxes[:, 1], boxes[:, 2] + boxes[:, 0]]).reshape((1, 4))
    fig, ax = plt.subplots(1)
    ax.imshow(im)
    for i in range(0, boxes.shape[0], step):
...
def show_res(im, box, win_name,confidence=None,score=None,save_path=None,frame_id=None,mask=None,score_max=None, groundtruth=None, can=None, local_state=None, rtbox=None):
    ...
    cv2.rectangle(im, (box[0], box[1]),
                  (box[2]+box[0], box[3]+box[1]), [0, 255, 255], 2)
